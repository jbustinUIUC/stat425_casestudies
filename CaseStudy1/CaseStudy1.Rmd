...
# We are dropping bachelors and high school below so no issues
```
...
##I switched model_data to be a dataframe which I believe solves this issue-Carrie -- changed back because model_data was a dataframe already but when it was passed in to the function it was adding back the index column and then the index column was getting passed into the model -- instead we can use the original data before we subset to the desired columns
...
```{r}
## Next Steps
## Address constant variance concerns + normality
## Assess/Address colinearity (this could be what is causing constant variance + normality issues)
## Finalize Model
## Write up results
```
